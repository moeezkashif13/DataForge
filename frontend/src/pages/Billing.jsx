...
import { CreditCard, CheckCircle2, Download, Zap, Shield, ArrowUpRight } from 'lucide-react'
    ...
    <div className="max-w-4xl space-y-8">
      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-xs space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">DataRelay Pro</h2>
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Production-grade control plane orchestration for enterprise workloads.
            </p>
          </div>

          <div className="flex items-baseline gap-1 text-right sm:text-right">
            <span className="text-3xl font-extrabold font-mono text-slate-900 dark:text-white">$99</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-4 border-t border-slate-100 dark:border-slate-800 text-xs">
          ...
          <div className="space-y-2">
            <div className="flex justify-between font-mono">
              ...
              <span className="font-semibold text-slate-900 dark:text-white">4.82M / 10M</span>
            </div>
            <div className="w-full h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
              <div className="h-full rounded-full bg-indigo-600" style={{ width: '48.2%' }} />
            </div>
            <p className="text-[11px] text-slate-400">48% of monthly allocation</p>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between font-mono">
              ...
              <span className="font-semibold text-slate-900 dark:text-white">3 / 20</span>
            </div>
            <div className="w-full h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
              <div className="h-full rounded-full bg-emerald-500" style={{ width: '15%' }} />
            </div>
            <p className="text-[11px] text-slate-400">17 concurrent slots available</p>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between font-mono">
              ...
              <span className="font-semibold text-slate-900 dark:text-white">5 / 10</span>
            </div>
            <div className="w-full h-2 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
              <div className="h-full rounded-full bg-cyan-500" style={{ width: '50%' }} />
            </div>
            <p className="text-[11px] text-slate-400">5 agent seats available</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
          <button
            type="button"
            onClick={() => showToast('Stripe Billing Portal', 'Redirecting to secure Stripe billing portal...', 'info')}
          ...
          <button
            type="button"
            onClick={() => showToast('Payment Method', 'Card on file ending in •••• 4242', 'info')}
      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-xs space-y-4">
        <h3 className="text-sm font-bold text-slate-900 dark:text-white">Payment Invoices</h3>
        <div className="divide-y divide-slate-100 dark:divide-slate-800 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden text-xs">
          {invoices.map((inv) => (
            <div key={inv.id} className="p-4 flex items-center justify-between hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => showToast('Receipt', `Downloading ${inv.id}.pdf...`, 'info')}
